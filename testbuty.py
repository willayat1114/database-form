...
def display_all_data(data):
    ...
    columns = (
        "ID", "Name", "Class", "Stream", "Phone Number", "Father's Name",
        "Mother's Name", "Parent's Phone", "Permanent Address",
        "Correspondence Address", "Photo"
    )
    ...
    # Display images within the treeview
    for row in data:
        ...
        if photo_path:
            try:
                ...
                tree.insert("", tk.END, values=values + [""], image=photo)
    ...
    # Keep a reference to the list of PhotoImage objects
    all_data_window.photo_images = photo_images
